...
from mbot.common.osutils import OSUtils, LinkMode
from mbot.core import EventListener, PluginContext
from mbot.core.events import Event, EventType
from mbot.external.mediaserver import MediaServerProxy
...
SOURCE_LIBRARY_PREFIX = '/media'
TARGET_LIBRARY_PREFIX = '/media/plex'
...
class MediaServerSync(EventListener, PluginContext):
    __bind_event__ = [EventType.DownloadCompleted]
    __order__ = 0
    ...
    def on_event(self, event: Event):
        ...
        # # 把硬连接好后的路径前缀做替换，替换为新媒体库需要硬连接的路径
        # new_target_path = target_path.replace(SOURCE_LIBRARY_PREFIX, TARGET_LIBRARY_PREFIX)
        # _LOGGER.info(f'新的{media_type}下载完了，内容路径：{target_path} 开始镜像到Plex')
        # if media_type == 'TV':
        #     season_number = data.get('season_number')
        #     episodes = data.get('episodes')
        #     # 对剧集做硬连接，同时只过滤符合条件的季度和集号
        #     OSUtils.link_folder(target_path, new_target_path, LinkMode.LINK, filter_season=season_number,
        #                         filter_episodes=episodes)
        # else:
        #     # 对电影的文件目录做直接硬连接
        #     OSUtils.link_folder(target_path, new_target_path, LinkMode.LINK)
        # media_server.refresh_media_server(tmdb_id, new_target_path, media_type)
